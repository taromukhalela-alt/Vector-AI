...
const handleDownloadPDF = async () => {
  ...
  try {
    ...
    // 1. Create a temporary element with a shadow root (complete style isolation)
    host = document.createElement('div');
    host.style.position = 'fixed';
    host.style.top = '-9999px';
    host.style.left = '-9999px';
    host.style.width = '794px';
    host.style.height = '1122px';
    host.style.background = '#ffffff';
    document.body.appendChild(host);

    const shadow = host.attachShadow({ mode: 'closed' });

    // 2. Write our own clean HTML/CSS inside the shadow (NO Tailwind can enter)
    shadow.innerHTML = `
    ...
    // 3. Inject the real content using shadow DOM selectors
    ...
    // Wait for KaTeX CSS + auto-render
    ...
    // 4. Export the shadow host as PDF (html2pdf sees the shadow content)
