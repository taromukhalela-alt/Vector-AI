...
const Slider = ({ label, unit, min, max, step, value, onChange }) => {
  ...
  const displayValue = Number.isFinite(numericValue)
    ? numericValue.toFixed(step < 0.1 ? 2 : step < 1 ? 1 : 0)
    : value;
  ...
  return (
    <div className="mb-4 flex flex-col gap-1">
      <div className="flex items-end justify-between">
        ...
        <span className="lab-readout border-2 border-[var(--border)] px-1 text-xs font-bold text-[var(--ink)] shadow-[1px_1px_0_var(--border)]">
          {displayValue}
          {unit}
        </span>
      </div>
...
const Lab = ({ activeAnim = 'idle', onAnimChange }) => {
  ...
  const labs = [
    ...
    },
    {
      ...
      icon: Rocket,
      color: 'bg-[var(--emerald)] text-[#0B1410]'
    },
    {
      id: 'wave',
      label: 'Wave motion',
      icon: WaveSine,
      color: 'bg-[var(--lab-aqua)] text-[#111]'
    },
    {
      id: 'pendulum',
      label: 'Harmonic pendulum',
      icon: Clock,
      color: 'bg-[var(--warning)] text-[#111]'
    },
    {
      id: 'forces',
      label: 'Forces & friction',
      icon: Cube,
      color: 'bg-[var(--lab-forge)] text-[#111]'
    },
    {
      id: 'collision',
      label: 'Collisions',
      icon: Circle,
      color: 'bg-[var(--magenta)] text-white'
    },
    {
      id: 'orbit',
      label: 'Orbits',
      icon: Planet,
      color: 'bg-[var(--lab-steel)] text-white'
    },
    {
      id: 'electricity',
      label: 'Electric field',
      icon: Lightning,
      color: 'bg-[var(--ink)] text-[var(--paper)]'
    },
  ];
    ...
    <div className="app-page-padded flex h-full flex-col overflow-y-auto bg-[var(--paper)] text-[var(--ink)]">
      ...
      <div className="mb-6 flex flex-col justify-between border-b-[3px] border-[var(--border)] pb-6 md:flex-row md:items-end">
        <div>
          <p className="section-label mb-2">
            Vector Lab Workspace · Scientific Instrumentation
          </p>
        ...
        <div
          className="hide-scrollbar mt-4 flex gap-2 overflow-x-auto pb-2 md:mt-0 md:pb-0"
          role="tablist"
      ...
      <div className="flex min-h-[500px] flex-1 flex-col gap-6 lg:grid lg:grid-cols-[1fr_300px]">
        ...
        <motion.div
          ...
          className="neo-panel relative flex min-h-[400px] flex-1 flex-col overflow-hidden"
        >
          {/* Internal HUD */}
          <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between p-4">
            <div className="border-2 border-[var(--border)] bg-[var(--surface)] p-2 shadow-[2px_2px_0_var(--border)]">
              <div className="mb-1 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[var(--ink)]">
                <span className="block h-2 w-2 border border-[var(--border)] bg-[var(--lab-aqua)]" />
                HUD
              </div>
              ...
              <p className="lab-readout text-xs font-bold text-[var(--ink-muted)]">
                {hudFormula}
              </p>
            </div>
            ...
            <div className="flex min-w-max items-center gap-6">
              ...
              {Object.entries(readout).length > 0 ? (
                Object.entries(readout).map(([key, value]) => (
                  <div
                    key={key}
                    className="lab-dial flex items-center gap-3 px-3 py-1.5"
                  >
                    <span className="text-[10px] font-black uppercase tracking-widest text-[var(--ink-muted)]">
                      {key}
                    </span>

                    <span className="lab-readout text-sm font-bold text-[var(--ink)]">
                      {String(value)}
                    </span>
                  </div>
                ))
              ) : (
                <div className="font-mono text-xs font-bold text-[var(--ink-muted)]">
                  AWAITING SIGNAL — RUN AN EXPERIMENT
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {activeAnim === 'idle' && (
              <p className="p-2 text-sm font-bold leading-relaxed text-[var(--ink-muted)]">
                Select an experiment from the workspace tabs to load its
                mechanical controls.
              </p>
            )}
            ...
            {activeAnim === 'projectile' && (
              <>
                ...
                <Slider
                  ...
                  onChange={(v) => handleParamChange('velocity', v)}
                />
                ...
                <Slider
                  ...
                  onChange={(v) => handleParamChange('bounciness', v)}
                />
                ...
                <Toggle
                  active={params.vectors}
                  onClick={() =>
                    handleParamChange('vectors', !params.vectors)
                  }
                  label="Show Vectors"
                />
              </>
            ...
            {activeAnim === 'forces' && (
              <>
                <Slider
                  ...
                  onChange={(v) => handleParamChange('applied', v)}
                />
                ...
                <Slider
                  ...
                  step={0.05}
                  value={params.mu}
                  onChange={(v) => handleParamChange('mu', v)}
                />
            ...
            {activeAnim === 'collision' && (
              <>
                <Slider
                  ...
                  step={0.5}
                  value={params.mass1}
                  onChange={(v) => handleParamChange('mass1', v)}
                />
                ...
                <Slider
                  ...
                  step={0.5}
                  value={params.mass2}
                  onChange={(v) => handleParamChange('mass2', v)}
                />

                <Toggle
                  active={params.elastic}
                  onClick={() =>
                    handleParamChange('elastic', !params.elastic)
                  }
                  label={
                    params.elastic
                      ? 'Elastic Collision'
            ...
            {activeAnim === 'orbit' && (
              <Slider
                ...
                onChange={(v) =>
                  handleParamChange('eccentricity', v)
                }
              />
            )}
            ...
            {activeAnim === 'electricity' && (
              <Slider
                ...
                onChange={(v) =>
                  handleParamChange('separation', v)
                }
              />
            )}

            <div className="mt-8 border-t-2 border-[var(--border)] pt-4">
              <Slider
                label="Time Scale"
                unit="x"
                min={0.25}
